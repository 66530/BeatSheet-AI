        ...
        if sys.platform == "win32":
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                if len(parts) >= 5 and "LISTENING" in line:
                    try:
                        pid = int(parts[-1])
                        if pid not in pids and pid != 0 and pid != 4:
                            pids.append(pid)
                    except ValueError:
                        pass
        else:
...
def kill_pids_on_port(port: int) -> int:
    ...
    for pid in pids:
        try:
            if sys.platform == "win32":
                # /T 杀死整个进程树 (children + grandchildren)
                subprocess.run(
                    ["cmd", "/c", f"taskkill /T /F /PID {pid}"],
...
def run_subprocess(cmd: list[str], cwd: Path, name: str) -> subprocess.Popen | None:
    """启动子进程并注册到全局列表（Windows 上使用进程组确保可以整树杀死）"""
    try:
        kwargs = {
            "cwd": str(cwd),
        ...
        if sys.platform == "win32":
            # CREATE_NEW_PROCESS_GROUP: 使子进程拥有独立的进程组，Ctrl+C 不会直接传给它
            kwargs["creationflags"] = subprocess.CREATE_NO_WINDOW | subprocess.CREATE_NEW_PROCESS_GROUP
        else:
            # start_new_session: 让子进程脱离当前终端会话
...
def start_backend() -> bool:
    ...
    if is_port_in_use(BACKEND_PORT):
        ...
        # 无响应 → 强制清理残留进程
        print_warn(f"端口 {BACKEND_PORT} 被残留进程占用，正在自动清理...")
        killed = kill_pids_on_port(BACKEND_PORT)
        if killed > 0:
            print_ok(f"已清理 {killed} 个占用端口 {BACKEND_PORT} 的残留进程")
        if is_port_in_use(BACKEND_PORT):
            print_err(f"端口 {BACKEND_PORT} 清理失败，请关闭占用程序后重试")
            return False
...
def start_frontend() -> bool:
    ...
    if is_port_in_use(FRONTEND_PORT):
        ...
        # 无响应 → 强制清理残留进程
        print_warn(f"端口 {FRONTEND_PORT} 被残留进程占用，正在自动清理...")
        killed = kill_pids_on_port(FRONTEND_PORT)
        if killed > 0:
            print_ok(f"已清理 {killed} 个占用端口 {FRONTEND_PORT} 的残留进程")
        if is_port_in_use(FRONTEND_PORT):
            print_err(f"端口 {FRONTEND_PORT} 清理失败，请关闭占用程序后重试")
            return False
...
def cleanup():
    """优雅关闭所有子进程（Windows 上使用 taskkill /T 杀死进程树）"""
    for proc in _processes:
        try:
            if sys.platform == "win32":
                # /T 杀死整个进程树，防止孙进程残留占用端口
                subprocess.run(
                    ["cmd", "/c", f"taskkill /T /F /PID {proc.pid}"],
                    capture_output=True, timeout=10,
                )
            else:
...
示例:
  ...
  python run.py --no-open       # 启动但不自动打开浏览器
    ...
    parser.add_argument(
        ...
        help="停止所有运行中的服务（释放端口 3000 和 8000）"
    )
    ...
    if args.stop:
        print_banner()
        print_step("停止所有服务并释放端口", "🛑")
        for port in [FRONTEND_PORT, BACKEND_PORT]:
            if is_port_in_use(port):
                killed = kill_pids_on_port(port)
                if killed > 0:
                    print_ok(f"端口 {port} 已释放 ({killed} 个进程)")
                else:
                    print_warn(f"端口 {port} 释放失败")
            else:
                print_ok(f"端口 {port} 空闲")
        print_step("完成", "✅")
        return 0
