...
export default function WorkspacePage() {
  ...
  const [jobState, setJobState] = useState<SharedJobState>({
    ...
    reviewStatus: "uploading", progressPct: 0, currentStep: "",
  ...
  const isProcessing = ["uploading", "analyzing", "generating"].includes(jobState.reviewStatus);
    ...
    <div className="animate-fade-in">
      ...
      {hasJob && isProcessing && (
        <div className="mb-4 console-panel animate-slide-up">
          ...
          <p className="text-[10px] text-[--nuss-muted] mt-2 border-t border-[--nuss-border]/30 pt-1.5">
            {jobState.currentStep}
          </p>
          <p className="text-[9px] text-[--nuss-muted] opacity-50 mt-0.5">
      ...
      <div className="min-h-[400px]">
        ...
        <div className={activeTab === "characters" ? "" : "hidden"}>
          <CharacterGraph characters={jobState.characters} reviewStatus={jobState.reviewStatus} />
        </div>
        <div className={activeTab === "screenplay" ? "" : "hidden"}>
          <ScreenplayViewer scenes={jobState.scenes} reviewStatus={jobState.reviewStatus} screenplayRaw={jobState.screenplay} />
        </div>
      </div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/v1/jobs/submit", {
        ...
        body: JSON.stringify({ file_text: fileText, file_type: file.name.split(".").pop()?.toLowerCase() || "txt", novel_title: file.name.replace(/\.[^.]+$/, ""), file_name: file.name }),
      });
        ...
        <div className={`blank-safe-card p-6 text-center ${isProcessing ? "opacity-40 pointer-events-none cursor-not-allowed" : "cursor-pointer"}`} onClick={() => !isProcessing && fileInputRef.current?.click()}>
          ...
          {file ? (
            <div className="space-y-2">
              <div className="w-10 h-10 mx-auto rounded-lg bg-green-500/20 flex items-center justify-center"><span>📄</span></div>
            ...
            <div className="space-y-2">
              <div className="w-12 h-12 mx-auto rounded-xl border-2 border-dashed border-[--nuss-border] flex items-center justify-center"><span className="text-xl">📤</span></div>
